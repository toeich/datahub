...
public class DatasinkEditor extends CustomComponent {
	...
	public DatasinkEditor() {
		...
		schemaTree.addActionHandler(new Handler() {
			...
			public Action[] getActions(Object target, Object sender) {
				DatasinkContainer container = (DatasinkContainer) ((Tree) sender).getContainerDataSource();
				if (target instanceof Input) {
					Input input = (Input) target;
					if (input.getType() == ItemType.XML_ELEMENT) {
						return (container.getParent(target) != null)
			...
			public void handleAction(Action action, Object sender, Object target) {
				if (target instanceof Input) {
					...
					if (action == addElement) {
						...
						parent.removeInput((Input) target);
					}
		...
		schemaTree.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if (itemId instanceof Input) {
					detailsPanel.setContent(new DatasinkInputEditor((Input) itemId));
				} else {
	...
	public void setDatasink(Datasink datasink) {
		schemaTree.setContainerDataSource(new DatasinkContainer(datasink));
	}
