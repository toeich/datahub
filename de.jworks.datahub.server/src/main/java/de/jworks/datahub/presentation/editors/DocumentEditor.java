...
public class DocumentEditor extends CustomComponent {
	...
	public DocumentEditor(final Dataset dataset) {
		buildMainLayout();
		setCompositionRoot(mainLayout);
		
		Document document2 = XMLUtil.parse(dataset.getContent());
		processSingle(document2.getDocumentElement(), dataset.getGroup().getSchema().getRootElement(), panelContent);
		
		okButton.addClickListener(new ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				fireEvent(new SaveEvent(DocumentEditor.this, dataset));
			}
		});
	...
	private void process(Node parent, Element element, VerticalLayout container) {
		if (element.getMaxOccurs() == null || element.getMaxOccurs() > 1) {
			...
			if (node == null) {
				parent.appendChild(node = node.getOwnerDocument().createElement(element.getName()));
			}
