...
public class ConnectorView extends CustomComponent implements View {
	...
	private BeanItemContainer<Datasink> datasinks = new BeanItemContainer<Datasink>(Datasink.class);
	...
	public ConnectorView() {
		...
		datasourcesTab.setMargin(new MarginInfo(true, false, true, false));
		
		datasourcesTable.addGeneratedColumn("actions", new ColumnGenerator() {
			@Override
			public Object generateCell(Table source, Object itemId, Object columnId) {
				final Datasource datasource = datasources.getItem(itemId).getBean();
				...
				removeButton.addClickListener(new ClickListener() {
					@Override
					public void buttonClick(ClickEvent event) {
						connector.getSchema().getDatasources().remove(datasource);
		...
		addDatasourceButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				datasource.getSchema().addOutput(new Output("output", ItemType.XML_ELEMENT));
				Window window = new Window();
				window.setWidth("80%");
				window.setHeight("80%");
				window.setModal(true);
		...
		datasinksTab.setMargin(new MarginInfo(true, false, true, false));
		
		datasinksTable.addGeneratedColumn("actions", new ColumnGenerator() {
			@Override
			public Object generateCell(Table source, Object itemId, Object columnId) {
				final Datasink datasink = datasinks.getItem(itemId).getBean();
				HorizontalLayout actions = new HorizontalLayout();
				actions.setSpacing(true);
				Button editButton = new Button();
				editButton.setStyleName("link");
				...
				editButton.addClickListener(new ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						window.setHeight("80%");
						window.setModal(true);
						window.setContent(new DatasinkEditor(datasink));
						UI.getCurrent().addWindow(window);
					}
				});
				actions.addComponent(editButton);
				...
				removeButton.addClickListener(new ClickListener() {
					@Override
					public void buttonClick(ClickEvent event) {
						connector.getSchema().getDatasinks().remove(datasink);
						refreshDatasinksTable();
					}
				});
				actions.addComponent(removeButton);
				return actions;
		...
		datasinksTable.setContainerDataSource(datasinks, Arrays.asList("name", "actions"));
		datasinksTable.setColumnExpandRatio("name", 1.0f);
		
		addDatasinkButton.addClickListener(new ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				final Datasink datasink = new Datasink();
				datasink.getSchema().addInput(new Input("input", ItemType.XML_ELEMENT));
				Window window = new Window();
				window.setWidth("80%");
				window.setHeight("80%");
				window.setModal(true);
				...
				window.addCloseListener(new CloseListener() {
					@Override
					public void windowClose(CloseEvent e) {
						connector.getSchema().getDatasinks().add(datasink);
					}
				});
				UI.getCurrent().addWindow(window);
			}
	...
	public void enter(ViewChangeEvent event) {
		try {
			...
			label.setValue(Messages.format(this, "label_value", connector.getName()));
	...
	private void refreshDatasinksTable() {
		datasinks.removeAllItems();
		datasinks.addAll(connector.getSchema().getDatasinks());
	}
